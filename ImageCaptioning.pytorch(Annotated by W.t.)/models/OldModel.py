...
class OldModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.linear = nn.Linear(self.fc_feat_size, self.num_layers * self.rnn_size) # 图像特征的转化feature to rnn_size
    ...
    def init_hidden(self, fc_feats):
        image_map = self.linear(fc_feats).view(-1, self.num_layers, self.rnn_size).transpose(0, 1) #前两维进行调换,符合lstm输入
        if self.rnn_type == 'lstm':
            return (image_map, image_map)
        else:
            return image_map

    def forward(self, fc_feats, att_feats, seq):#seq就是label一句话 batch以后  词数*i * seq_length
        batch_size = fc_feats.size(0)
        state = self.init_hidden(fc_feats) #把图像特征映射到hidden层大小

        outputs = []

        for i in range(seq.size(1) - 1):#每个词
            if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
                ...
                if sample_mask.sum() == 0:
                    ...
                    sample_ind = sample_mask.nonzero().view(-1) #返回不是0的索引
                    it = seq[:, i].data.clone()
                    #prob_prev = torch.exp(outputs[-1].data.index_select(0, sample_ind)) # fetch prev distribution: shape Nx(M+1)
                    #it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1))
                    prob_prev = torch.exp(outputs[-1].data) # fetch prev distribution: shape Nx(M+1)
                    #输出中每行随机取一个展平,按照sample_ind挑出来
            ...
            xt = self.embed(it)#词的嵌入

            output, state = self.core(xt, fc_feats, att_feats, state) #这里的state是有参数的?
            output = F.log_softmax(self.logit(self.dropout(output)))
            outputs.append(output)#outputs是每个词的概率
...
class ShowAttendTellModel(OldModel):
    def __init__owAttendTellModel, self).__init__(opt)
